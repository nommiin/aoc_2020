...
5-6 g: kgggjg
4-8 s: ssssssscs
12-20 t: ttttttltttvttttttttt`.split("\n");

function ParsePassword( _input ) {
    var _policyIndex = _input.indexOf(":");

    this.Password = _input.slice(_policyIndex + 1).trim();
    this.Letter = _input.slice(_policyIndex - 1, _policyIndex).trim();
    this.Policy = _input.slice(0, _policyIndex - 1).trim().split("-");
    for(var i = 0; i < 2; i++) this.Policy[i] = parseInt(this.Policy[i]);

    // part 2
    var _one = (this.Password[this.Policy[0] - 1] == this.Letter) ? 1 : 0;
    var _two = (this.Password[this.Policy[1] - 1] == this.Letter) ? 1 : 0;
    if (_one + _two == 1) return true;
    return false;

    // part 1
    var _letterCount = 0;
    for(var i = 0; i < this.Password.length; i++) {
        if (this.Password[i] == this.Letter) _letterCount++;
    }
    if (_letterCount >= this.Policy[0] && _letterCount <= this.Policy[1]) return true;
    return false;
...
for(var i = 0; i < input.length; i++) {
    if (ParsePassword(input[i]) == true) {
        _validCount++;
    }
}
console.log("Valid = " + _validCount);